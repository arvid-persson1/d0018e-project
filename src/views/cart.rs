            ...
            div { class: "max-w-4xl mx-auto p-6",
                ...
                if checkout_done() {
                    ...
                    match &*cart_resource.read() {
                        None => rsx! {
                            p { class: "text-gray-400 animate-pulse", "Laddar kundvagn..." }
                        },
                        Some(None) => rsx! {
                            p { class: "text-red-400", "Kunde inte hämta kundvagnen." }
                        },
                        Some(Some(products)) if products.is_empty() => rsx! {
                        ...
                        Some(Some(products)) => {
                            let total: rust_decimal::Decimal = products
                                .iter()
                                .map(|p| {
                                    let effective_price = p
                                        .special_offer_deal
                                        .as_ref()
                                        .map(|d| d.average_discount(p.price))
                                        .unwrap_or(p.price);
                                    effective_price * rust_decimal::Decimal::from(p.count.get())
                                })
                                .sum();
                            rsx! {
                    ...
                    // Ej inloggad – visa lokal kundvagn
                    {
