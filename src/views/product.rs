...
fn ReviewsSection(
    ...
    let rev_data: Option<(Option<OwnReview>, Vec<ProductReview>)> = rev_read.as_ref()
        .and_then(|r| r.as_ref().ok())
        .map(|(own, others)| (own.clone(), others.clone()));
    let rev_has_own    = rev_data.as_ref().map(|(own, _)| own.is_some()).unwrap_or(false);
    let rev_no_content = rev_data.as_ref().map(|(_, others)| others.is_empty()).unwrap_or(false) && !rev_has_own;
    ...
    || review_content().trim().is_empty() { "bg-gray-200 text-gray-400 px-5 py-2 rounded-full text-sm font-bold cursor-not-allowed" } else { "bg-green-700 text-white px-5 py-2 rounded-full text-sm font-bold hover:bg-green-800 transition" },
            ...
            } else if let Some((own_review, other_reviews)) = rev_data {
                ...
                if rev_no_content {
                    div { class: "text-center py-12 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200",
                        ...
                        p { class: "text-gray-300 text-sm mt-1",
                            "Bli den första att recensera produkten."
                        }
                    }
                } else {
                    div { class: "space-y-4",
...
fn SimilarProducts(props: SimilarProductsProps) -> Element {
    ...
    let sim_loading = sim_read.is_none();
    let sim_err: Option<String> = sim_read.as_ref()
        .and_then(|r| r.as_ref().err().map(|_| "Kunde inte hämta liknande produkter.".to_string()));
    let sim_items: Option<Vec<_>> = sim_read.as_ref()
        .and_then(|r| r.as_ref().ok())
        .map(|v| v.to_vec());
    let sim_empty = sim_items.as_ref().map(|v| v.is_empty()).unwrap_or(false);
 
    if sim_loading {
        rsx! {
            p { class: "text-gray-400 animate-pulse", "Laddar liknande produkter..." }
        }
    } else if sim_err.is_some() {
        rsx! {
    ...
    } else if let Some(items) = sim_items {
        rsx! {
...
pub fn Product(id: i32) -> Element {
    ...
    // Resolve outside rsx! — early returns for loading/error
    ...
    let offer_label_str: Option<String> = match product.special_offer_deal.and_then(|d| d.database_repr().ok()) {
            ...
            div { class: "grid grid-cols-1 md:grid-cols-2 gap-12 mb-16",
                ...
                div { class: "flex flex-col justify-start",
                    ...
                    div { class: "border-t border-b py-6 mb-6",
                        ...
                        if let Some(lbl) = offer_label_str {
